...
| [0094](./0094-explicit-per-pane-scrollback-byte-ceiling.md) | Per-pane scrollback is bounded in bytes, by phux, explicitly | Accepted (`defaults.history-limit` is only libghostty's line limit and the engine's own 10_000-byte constructor default was what actually bound retention, so `TerminalActor::build` installs a 2 MiB byte ceiling; retention trades directly against attach latency because the native bootstrap materialises every retained page at READY) |
