...
- `remote-tokens` is the versioned verifier-only credential store the server reads and `phux pair` updates under the sibling `.remote-tokens.lock`, using a synced temporary file and atomic rename; `PHUX_WS_TOKENS` moves it. Legacy anonymous token lines require the idempotent `phux pair --migrate-legacy` conversion.
