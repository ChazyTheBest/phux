...
| `PHUX_FRAME_INTERVAL_MS=<ms>` | Client only. Minimum interval between composited frames; default `16` (one frame at 60Hz). The first frame after any lull always paints immediately, and output arriving within 20 ms of the user's own input is never paced at all, so this only bounds how often a sustained *unsolicited* output stream repaints. `0` disables pacing entirely. |
